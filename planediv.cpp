  ...
  while(cases--){
    ...
    for(int i=0; i<lines; i++){
      ...
      for(int j=i; j<lines; j++){
        ...
        cout << endl;
        if(line_set[i][0] == 0 && line_set[j][0] == 0){
          if((line_set[i][1] == line_set[j][1]) && (line_set[i][2]*line_set[j][1] != line_set[j][2]*line_set[i][1])) count++;
          continue;
        }
        else if(line_set[i][1] == 0 && line_set[j][1] == 0){
          if((line_set[i][0] == line_set[j][0]) && (line_set[i][2]*line_set[j][0] != line_set[j][2]*line_set[i][0])) count++;
          continue;
        }

        if(line_set[i][0]*line_set[j][1] == line_set[i][1]*line_set[j][0] && ((line_set[i][2]*line_set[j][0] != line_set[j][2]*line_set[i][0]) || (line_set[i][2]*line_set[j][1] != line_set[j][2]*line_set[i][1]))) count++;
      }
